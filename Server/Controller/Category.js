...
/**
 * CREATE CATEGORY
 * → invalidates cached category data
 */
exports.createCategory = async (req, res) => {
  try {
    const { name, description } = req.body;
...
/**
 * GET ALL CATEGORIES
 * Cached (read-heavy endpoint)
 */
exports.showAllCategories = async (req, res) => {
  try {
    const cacheKey = cacheService.generateKey("category", "all");

    const categories = await cacheService.cacheOrExecute(
      cacheKey,
      async () => {
        return await Category.find().populate("courses");
      },
...
exports.categoryPageDetails = async (req, res) => {
  try {
    ...
    const cacheKey = cacheService.generateKey("category", "page", categoryId);
